...
namespace UnityUtils.Extensions
{
    public static class ArrayExtensions
    {
        ...
        public static T[] AddList<T>(this T[] target, T[] newItems)
        {
            T[] result = new T[target.Length + newItems.Length];
            for (int i = 0; i < target.Length; i++)
                result[i] = target[i];
            
            for (int i = 0; i < newItems.Length; i++)
                result[target.Length + i] = newItems[i];
        ...
        public static T[] ClearAllNulls<T>(this T[] list)
        {
            if (list == null)
            {
                return Array.Empty<T>(); // Eğer dizi null ise, boş bir dizi döner
            }

            // null olmayan elemanları filtreleyerek yeni bir dizi oluşturur
            return list.Where(item => item != null).ToArray();
        }
